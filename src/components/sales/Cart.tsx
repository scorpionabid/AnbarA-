...
import { ShoppingCart, Search, Package, Minus, Plus, X, Save, ChevronRight, Loader2, History } from "lucide-react";
...
interface CartItem {
  ...
  price: number;
  quantity: number;
  purchasePrice?: number; // Added for purchase compatibility if needed
}
...
interface CartProps {
  ...
  setChannel: (channel: "online" | "offline") => void;
      ...
      <div className="relative flex flex-col h-full">
        <div className="flex justify-between items-center mb-6">
          <div className="flex items-center gap-2">
            ...
            <h3 className="font-medium text-lg">Səbət</h3>
          </div>
                ...
                >
                  <div className="flex-1 pr-4">
                    ...
                    <div className="flex items-center gap-1 mt-0.5">
                      <span className="text-zinc-400 text-xs text-nowrap">₼</span>
                      {updatePrice ? (
                        <input
                          type="number"
                          value={item.price}
                          onChange={(e) => updatePrice(item.id, parseFloat(e.target.value) || 0)}
                          className="w-16 bg-transparent border-b border-dashed border-zinc-300 text-xs text-zinc-900 focus:outline-none focus:border-zinc-500"
                          step="0.01"
                          min="0"
                        />
                      ) : (
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <span className="font-semibold text-sm text-zinc-900 w-16 text-right">₼{(item.price * item.quantity).toFixed(2)}</span>
                    <button 
        ...
        <div className="space-y-5">
          ...
          <div className="flex gap-2">
            <div className="flex-1 flex p-1 bg-zinc-50 rounded-xl border border-zinc-100">
              <button onClick={() => setChannel("offline")} className={cn("flex-1 py-1.5 text-xs font-medium rounded-lg transition-all", channel === "offline" ? "bg-white text-zinc-900 shadow-sm" : "text-zinc-500 hover:text-zinc-700")}>Mağaza</button>
              <button onClick={() => setChannel("online")} className={cn("flex-1 py-1.5 text-xs font-medium rounded-lg transition-all", channel === "online" ? "bg-white text-zinc-900 shadow-sm" : "text-zinc-500 hover:text-zinc-700")}>Onlayn</button>
            </div>
          </div>
          ...
          <div className="flex p-1 bg-zinc-50 rounded-xl border border-zinc-100">
            ...
            <button onClick={() => { setPaymentMethod("credit"); onOpenClientModal(); }} className={cn("flex-1 py-1.5 text-xs font-medium rounded-lg transition-all", paymentMethod === "credit" ? "bg-white text-zinc-900 shadow-sm" : "text-zinc-500 hover:text-zinc-700")}>Nisyə</button>
          </div>

          {paymentMethod !== "credit" && cashboxes && cashboxes.length > 0 && setSelectedCashbox && (
            <div className="flex flex-col gap-1.5">
              <label className="text-xs font-medium text-zinc-500 px-1">Ödənişin daxil olacağı kassa</label>
              <select
          ...
          <div className="flex items-center gap-3 pt-4 mt-2">
            ...
            <button disabled={cart.length === 0 || isProcessing} onClick={onCompleteSale} className="flex-1 bg-zinc-900 text-white py-3 px-4 rounded-xl font-medium hover:bg-zinc-800 transition-all disabled:opacity-50 disabled:cursor-not-allowed active:scale-95 flex items-center justify-between group shadow-sm">
              ...
              <div className="space-y-2">
                {recentSales.slice(0, 3).map((sale) => (
                  <div key={sale.id} className="flex justify-between items-center text-xs">
                    <div className="flex items-center gap-2">
                      <span className="text-zinc-400">{sale.createdAt?.toDate().toLocaleTimeString('az-AZ', { hour: '2-digit', minute: '2-digit' })}</span>
                      <div className={cn(
                    ...
                    <span className="text-zinc-900 font-medium">₼{sale.totalAmount.toFixed(2)}</span>
                  </div>
                ))}
              </div>
            </div>
