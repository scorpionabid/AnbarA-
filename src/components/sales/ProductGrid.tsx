    ...
    <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-4">
      {products.map((p) => (
        <motion.button
          ...
          disabled={p.stock <= 0}
          className={`group relative flex flex-col bg-white border border-zinc-200 rounded-[1.5rem] overflow-hidden hover:border-zinc-300 hover:shadow-xl hover:-translate-y-1 transition-all duration-300 text-left ${p.stock <= 0 ? 'opacity-50 grayscale cursor-not-allowed' : 'active:scale-95'}`}
        >
